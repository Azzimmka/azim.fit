...
test('открытое меню карточки остаётся поверх следующей тренировки', async ({ page }) => {
  await loadDemo(page);
  ...
  const actionMenu = original.locator('details.action-menu');
  if (!(await actionMenu.evaluate((element) => element.open))) {
    await original.getByRole('button', { name: 'Действия: Верх тела' }).click();
  }
  const deleteButton = original.getByRole('button', { name: 'Удалить' });
  await deleteButton.scrollIntoViewIfNeeded();
  await expect(deleteButton).toBeVisible();

  const isTopmost = await deleteButton.evaluate((button) => {
